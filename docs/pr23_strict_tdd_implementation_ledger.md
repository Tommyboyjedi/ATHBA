...
## Session 8B3B: application-level run controller

- Added typed start/resume request and result, durable controller state, bounded process transitions, lifecycle-event coordination, checkpoint result handling, stall detection, and deterministic report writing.
- The controller delegates progression to the Session 8B2 feature service; it adds no CLI, endpoint composition, Rack AI invocation, or live proof.
- INCOMPLETE_ITEMS = PRESENT: live composition/proof work remains outside this session.
