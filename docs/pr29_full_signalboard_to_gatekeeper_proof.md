...
## 2026-09-05: deterministic pytest probe-output harness correction

Historical run `pr29-fresh-signalboard-20260905T172413Z` stopped before Frontier classification. Its isolated subprocess reproduction produced a non-zero exit, zero stdout bytes, and pytest/Django startup stderr. The adapter incorrectly indexed `completed.stdout.splitlines()[-1]`, raising `IndexError` before recording a diagnostic. A new regression first reproduced that exact structural crash against starting head `4191f0da048575c813814d6fa95154cfe4cff97a` with controlled subprocess output. Non-JSON output also reproduced lost execution evidence; JSON null/arrays raised `AttributeError`, and an incomplete passed object incorrectly reached GREEN.

The narrow adapter correction checks subprocess termination and validates the final JSON record and its protocol fields before interpreting test outcomes. Empty or whitespace-only stdout, non-JSON/truncated output, malformed structured fields, and non-zero probe termination now return the existing typed `BoundaryDiagnostic(kind="infrastructure")`, classified as `BoundaryOutcome.INFRASTRUCTURE_FAILURE`. Diagnostic facts retain the probe return code and JSON-encoded stdout, stderr, and command (including root and test-node context); encoding preserves empty/whitespace output through the nonblank diagnostic-fact contract. Environment mappings are not included. Deterministic microcycle regression confirms blocked state, no accepted RED/GREEN, no Developer submission, no regression execution, no Frontier/revision advancement, and no promotion. Existing RED classifier code is unchanged.

Validation: 32 directly invoked deterministic regression cases passed, covering malformed/empty output, diagnostic serialization, fail-closed microcycle behavior, valid structured parsing, missing-capability RED (including assertions), ordinary assertion RED, GREEN, and genuine unsupported-language boundaries. These direct invocations are not a full pytest-suite pass. Focused pytest and normal `./.venv/bin/python -m pytest -q` were both attempted and stopped during pytest-django startup with `ImportError: Set the DJANGO_SECRET_KEY environment variable`, before test collection. Coding-principles, configured mypy (29 files), explicit adapter mypy, `python -m compileall core tests scripts`, and `git diff --check` passed. Detailed new validation logs are under untracked `evidence/pr29-pytest-probe-output-fix/`.

The Django launch/preflight issue remains separate and was **not fixed**: no Django settings, secret, environment provisioning, plugin configuration, generated project, or proof script was changed. No historical run or evidence was altered or resumed; prior document bytes remain an exact prefix. No new SignalBoard proof and no model calls (`MODEL_CALLS_MADE=0`). No TDD semantic, prompt, grammar, Frontier machinery, budget, timeout, routing, Rack AI, JCode, BPQ-V1, or PR21 changes. Normal pytest validation remains blocked pending an authorized test environment.

### Authorized CI-only validation closure

Following explicit authorization to use the repository's existing CI test variables only for deterministic validation commands, the fix at `f40b5a7d2151a3373d4a087c0d44441c5f748e19` passed normal pytest validation: **79 focused tests passed** (`tests/development/test_python_pytest_adapter.py` and `tests/development/test_strict_microcycle.py`, 44.17 seconds), and **650 full-suite tests passed** (`python -m pytest -q`, 168.36 seconds). Coding-principles, configured mypy (29 files), `python -m compileall core tests scripts`, and `git diff HEAD^ HEAD --check` also passed. Logs are retained under untracked `evidence/pr29-pytest-probe-output-fix/ci-*.txt`. The earlier blocked attempts remain historical evidence; this authorized validation resolves that validation blocker.

The empty/malformed-output regressions still control `subprocess.run` deterministically and passed with the CI environment present. They do not depend on a missing `DJANGO_SECRET_KEY` or a real pytest/Django failure. No additional adapter or TDD changes were needed. CI variables were scoped to individual validation commands, not persisted or exported into the shell. No configuration, provisioning, Django settings, proof scripts, or generated project was changed. The separate Django launch/preflight issue remains **unfixed**. No model calls (`MODEL_CALLS_MADE=0`), no live SignalBoard proof, no historical-state changes, and no PR29 merge occurred.

## 2026-09-05: isolate target pytest discovery and gate live composition

The Django contamination seen in historical run `pr29-fresh-signalboard-20260905T172413Z` was an ATHBA **probe-isolation defect**, not a requirement for generated non-Django projects to acquire ATHBA settings. Before production edits at `5a6c8113016472548a41269fdaf281c41c21e1fb`, a real adapter/subprocess regression with an equivalent enclosing Django harness failed on that harness's settings. A separate instrumented reproduction under `evidence/pr29-pytest-isolation/target-before` established: probe cwd was already the target, but `sys.path` retained two `/srv/ATHBA` entries; adapter `PYTHONPATH` was `/srv/ATHBA`; pytest selected root `/srv/ATHBA` and `/srv/ATHBA/pytest.ini`, whose `DJANGO_SETTINGS_MODULE` is `athba.settings`; pytest-django was autoloaded. `DJANGO_SECRET_KEY` and inherited settings selection were absent. The resulting typed infrastructure diagnostic retained the zero stdout/non-zero exit and Django startup stderr.

The exact coupling is two independent ancestor searches plus bootstrap import exposure. Pytest's config search ascends from the nested target into the enclosing harness. The installed pytest-django `_add_django_project_to_path` searches test arguments and the target cwd's parents for `manage.py`; it does **not** discover Django by scanning `sys.path`, and rootdir/confcutdir alone do not bound that plugin search. Its `_initialize_django` then selects settings from the inherited pytest ini and forces settings loading. The `-m` launch and adapter PYTHONPATH make the harness importable but are not themselves the ancestor-discovery algorithm. Relevant implementation was read directly from the installed pytest/pytest-django runtime; before/after discovery records and diagnostics are retained under untracked `evidence/pr29-pytest-isolation/`.

`TargetPytestEnvironment` now separates bootstrap from target discovery. After harness collaborators are imported, it removes the harness root from import paths/PYTHONPATH and releases bootstrap `core` package registrations while retaining the already-imported collaborators. It clears inherited host pytest/Django control selectors, keeps runtime site-packages and third-party plugin autoload, selects the nearest valid target-owned pytest config without walking above the target root, and uses an explicit empty config only when none exists. Explicit rootdir/confcutdir bound pytest/conftest discovery. Pytest-django's unbounded manage.py scan is replaced with equivalent import-path assistance bounded to the target (honoring a target's disabled project-search setting); the Django plugin itself remains enabled. Tests preserve target pytest.ini/pyproject settings, conftest fixtures, target plugins, target package imports, and legitimate target-declared Django settings. No Django secret is injected. A real post-fix subprocess without DJANGO_SECRET_KEY uses the target root, `/dev/null` fallback config, no harness root import entry, and no ATHBA settings, with pytest-django still loaded. The structured facts protocol, missing-member classifier, and previous empty/malformed/non-zero-output fail-closed handling are unchanged.

`PythonPytestPreflight.check(workspace_parent)` is the reusable zero-model readiness primitive for the adapter's current Python/pytest runtime. It executes a disposable test that requires no production behavior and checks target/config/import isolation as well as complete successful collection/setup/call/teardown facts. Startup errors, missing runtime, timeout, absent/malformed output, or failed readiness assertions return typed infrastructure diagnostics. `StrictTddLiveRunCompositionFactory` runs this before constructing live reasoning, feature orchestration, or execution gateways; the CLI reports a typed `python_pytest_preflight_failed` blocker with exit 2 on failure, before run/trusted-revision state is created. Fake-only failure tests prove no Planner/Tester/Intent/Developer composition, reasoning request, or Rack AI model submission, and no trusted revision progression. Success permits the existing orchestration, whose deterministic checkpoint/recovery proof still passes. This is runtime/isolation readiness, not proof of future generated behavior or future project-specific dependencies. Future live proofs must pass this primitive **before any model-backed call, including a separate model-readiness generation**; the production runner enforces it at live composition. No live proof was run here.

Validation: **109 focused tests passed** (65.02s), then the final inherited-plugin-control boundary checks passed **14 tests** (3.29s); **664 full-suite tests passed** (142.72s). Coding-principles, configured mypy (29 files), explicit mypy on both new modules, `python -m compileall core tests scripts`, and `git diff --check` passed. A standalone zero-model preflight also passed without Django variables. ATHBA-suite CI variables were command-scoped only; no settings/provisioning/secret persistence changed. Prior document bytes and historical run evidence remain untouched. `MODEL_CALLS_MADE=0`; no SignalBoard proof, Behavior Planner/Tester/Intent/grammar/Frontier/RED/Developer/regression/Gatekeeper semantics, budget, timeout, routing, Rack AI, JCode, BPQ-V1, PR21, or merge changes.

## Final reconciliation output correction (2026-09-06; deterministic only)

Historical run `pr29-fresh-signalboard-20260905T220702Z` reached final reconciliation after all five planned Behavior Requirements. The supplied live evidence records canonical SHA `51e667f4a0b0c51aedc999deae67dc147943e553`, pending action `reconciliation`, 255 application transitions, and invocation count 2. Malformed reconciler text raised generic `ValueError("reconciler response was not valid JSON")`; the CLI rendered `invalid_input`, while the controller cleared its in-flight marker without changing `running`. The historical run, project, state and evidence were not modified or resumed.

At starting ATHBA head `d311b4011a112d5a32f75e24f6e94be67757dd8b`, fake-gateway baseline tests reproduced malformed/prose/fenced/wrong-shape output, CLI misclassification and false-running persistence. Together with existing verified YES/NO and Git evidence rejection cases, the baseline passed 13 tests before implementation. Those defect assertions were then replaced by corrected-behavior regressions.

The correction defines `ReconciliationResponse`: exactly one object with required `answer` (YES/NO), `selected_test_names` (array of strings) and `rationale` (string), with no extra fields. A NO cannot claim evidence. Claimed YES still passes through the unchanged deterministic verifier: empty, invented, missing or changed accepted tests yield verified NO. `GitAcceptedTestCatalog`, accepted-test collection, independent checklist meaning and the behavioral evidence standard remain authoritative and unchanged.

Generic reasoning and provider behavior is unchanged. No schema field is added to `ReasoningRequest`, no schema is forwarded to ProviderRequest, and OpenAIProvider request encoding/validation is untouched. The response contract is enforced locally inside reconciliation. Structured-response propagation and a Responses encoding correction were removed during pre-commit scope review because none is required to handle this reconciliation defect.

Reconciliation permits one original gateway submission and at most one format-only repair submission for non-JSON text, following the existing Senior-review bounded pattern without changing Senior Review. The repair receives only the previous output, schema and preservation instructions; it receives no requirement, accepted-evidence catalogue, production code or other pipeline context. A recovered single JSON fence must preserve the complete prior decision exactly. Arbitrary prose/broken JSON cannot be promoted to a decision merely because the repair is valid JSON: when preservation cannot be proved, reconciliation fails closed. Invalid schema/semantic decisions are not reconsidered. An exception from the reasoning gateway is wrapped at the reconciliation submission boundary as a separately typed provider/gateway failure and never initiates format repair. Other reasoning callers and their exception behavior are unchanged. Counts describe gateway submissions, not unobservable low-level HTTP retries.

`ReconciliationFailure` distinguishes malformed output, schema violations, invalid semantic answers and provider failure. It records the checklist identity, supplied accepted test IDs, submission/repair outcomes, response digests when available and any earlier completed checklist results, without recording raw malformed output or provider credentials. Successful results retain submission diagnostics, supplied test IDs and final verified YES/NO.

The feature application persists this failure with the existing blocked state and emits its ordinary typed feature-blocked transition. The unchanged controller then durably delivers the receipt, appends `feature_blocked` and `run_blocked`, produces reports, clears in-flight state and returns CLI `blocked` (exit 2). Completed behavior references, accepted scenario/microcycle/revision files, trusted SHA, independent checklist and pending reconciliation identity are preserved. The existing blocked-run policy is terminal on ordinary resume: a new controller returns blocked without invoking the application or repeating Planner, Tester, Frontiers, Developer, regression or Senior Review. This correction does not invent an unblock/recovery command or authorize altering historical state.

Validation includes a disposable real-Git feature driven exclusively by fake gateways, failure on its second checklist item, preserved first-item NO, byte-for-byte completed-state comparisons and a fresh CLI resume with zero execution/reasoning calls. No live proof, historical resume or model call occurred during this correction. `MODEL_CALLS_MADE=0`. Targeted qualification against separately copied/frozen evidence remains the next proof step; final fresh end-to-end proof readiness has not been established.

## 2026-09-06: exhausted Behavior Requirement recovery (deterministic implementation)

Starting head: `d1f6ec4f939799e84ae8261bf61835295a92080d`, branch `pr29-live-tiny-strict-tdd-v2-proof`. The supplied observation of fresh run `pr29-fresh-signalboard-20260906T002411Z` records successful completion of REQ-001 through REQ-004. REQ-005 (`PR16-007`, `PR16-008`, Architectural constraints; component is in-memory and dependency-free) correctly exhausted all four Tester attempts, including a timeout and valid candidates rejected by Intent for insufficient evidence. Rejecting inadequate evidence was correct. Immediate feature termination exposed a missing Behavior Planner recovery layer. This implementation does not resume or modify that run or `pr29-fresh-signalboard-20260905T220702Z`.

The policy is **four attempts -> parent-only Planner replan -> split and recurse -> unsplittable -> human blocker**. Tester attempt 5 is never permitted. The original Tester cap and submission path are unchanged. Only exhaustion supported by substantive candidate/semantic rejection evidence requests decomposition; pure infrastructure exhaustion, harness failure, or Intent protocol failure retains existing failure handling. A mixture of semantic rejections and a model timeout can request replanning. No SignalBoard names, source refs, architectural categories, or fixture conditions participate in this classification.

A typed `BehaviorReplanRequest` snapshots the original source requirement, parent (all normal requirement fields), source clauses, all four full typed draft attempts (mechanical feedback, candidate/Intent evidence, timeout/no-candidate facts), completed requirements, trusted canonical ref/SHA, and ancestor lineage. It explicitly prohibits changing completed behavior or redesigning the contract. The existing Behavior Planner owns the new reasoning purpose through the existing reasoning gateway; no other stage decomposes requirements.

`BehaviorReplanResponse` is either `split` with at least two children, a collective coverage/no-additions rationale, and per-child narrowing/source-grounding rationales, or `unsplittable` with a rationale. Children reuse `BehaviorContractRequirement`. The wire contract excludes IDs and dependencies: ATHBA allocates deterministic IDs such as `REQ-005-S001` and `REQ-005-S002`, inheriting dependencies. Dots are deliberately avoided because JSON repositories use filesystem suffix replacement. Recursive children use the same suffix rule. Existing IDs are never renumbered.

Structural checks reject unchanged parent/ancestor/existing outcomes, duplicate outcomes including reordered tokens, children repeating the full parent outcome, invalid or dropped source refs, identity collisions, altered inherited dependencies, dropped state-preservation/error obligations, and repeated split structure digests. Required rationales express the Planner's semantic judgment; deterministic code does **not** prove deep semantic narrowing, grounding, equivalence or completeness. Unchanged Intent, strict TDD, Senior Review and final independent Gatekeeper remain the evidence boundaries. There is one Planner semantic submission per exhausted parent, with no hidden repair/review loop. Malformed or invalid responses fail closed with the response and typed human-inspectable reason.

The typed safety policy bounds the entire tree at 256 splits and each split at 32 children by default, rather than imposing a tiny recursion depth. The composition accepts an explicit policy. Reaching a safety budget produces `behavior_unsplittable` and a durable human-escalation explanation. This does not increase or change any transition limit.

Feature JSON durably records required, started, response-received, superseded/split, unsplittable and failed phases. Request evidence is saved before invoking the Planner; a received response is saved before queue mutation. The subsequent atomic feature update archives the parent in its replan record, inserts children at its position, rewires pending dependencies, and clears only the active scenario selection. The record retains parent and child refs, split depth, rationale, source refs, raw response or rejected response, structural digest and trusted revision. Interrupted submissions without a durable response block for human reconciliation instead of issuing a second Planner request. Resume from received/split state uses the saved children and the same identities. Existing controller in-flight recovery policy remains authoritative.

Replanning performs no repository mutation, rollback, scenario reset or promotion. Completed references, accepted tests, Senior evidence, canonical revision and completed scenario states remain intact. Children execute through the existing Tester -> Intent -> deterministic fragmentation -> RED -> Developer -> GREEN -> regression -> Senior pipeline, with independent four-attempt draft state. They finish before requirements that followed the parent. The independent Gatekeeper checklist is not regenerated or edited: final reconciliation consumes completed child tests using the unchanged accepted-test collector and Git verifier against the original source checklist.

Typed feature transitions and the existing lifecycle projector expose replan-required, split-received, split and unsplittable events; started state is persisted before the external call. The normal proof report includes the replan records and archived parent draft evidence. No parallel event system or human UI was added. All implementation tests use fake reasoning/execution gateways and disposable repositories. `MODEL_CALLS_MADE=0`; no model readiness probes or live SignalBoard calls were made. No Rack AI, JCode, routing, gateway/provider architecture, Tester prompt/budget, Intent semantics, TDD, Developer, regression, Senior, Gatekeeper, pytest materialisation or PR21 changes are part of this implementation. Changes remain uncommitted and unpushed pending scope review.


## 2026-09-06: SignalBoard final Gatekeeper evidence and completion correction

Fresh run pr29-fresh-signalboard-20260906T105103Z completed six Behavior Requirements at final canonical revision c56a9556f42adf3b6460f62cb149407cbeabff21. Its final reconciliation returned 1 YES / 9 NO, but inspection showed that the Gatekeeper received accepted-test metadata only, not the actual test bodies; several rationales contradicted the preserved tests. The outer feature was incorrectly marked completed despite NO items.
