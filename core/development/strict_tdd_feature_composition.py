...
from core.development.strict_microcycle import GitFrontierMaterialiser, StrictMicrocycleDependencies, StrictMicrocycleService
...
class StrictTddFeatureCompositionFactory:
    ...
    def build(self, request: StrictTddCompositionRequest) -> StrictTddFeatureComposition:
        ...
        repair = BehaviorRepairService(BehaviorRepairDependencies(microcycle_store, BehaviorRepairGitCandidateRepository(candidates), gateway, regression))
        strict = StrictMicrocycleService(StrictMicrocycleDependencies(microcycle_store, candidates, gateway, adapters, regression, behavior_completion=completion, behavior_repair=repair))
        drafting = ScenarioDraftingService(ScenarioDraftingDependencies(gateway, ScenarioIntentReviewer(request.reasoning_gateway), adapters, GitCandidateScenarioSourceReader(request.repository_root), ScenarioDraftStateRepo(root / "scenario-drafts")))
