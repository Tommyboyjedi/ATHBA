...
def prepare_target(environment: ProjectEnvironmentService, project: DevelopmentProject) -> tuple[DevelopmentProject, list[str]]:
    ...
    probe_template = Path(__file__).with_name("assert_test_fails_template.py").read_text(encoding="utf-8")
    (target / "scripts" / "assert_test_fails.py").write_text(probe_template, encoding="utf-8")
